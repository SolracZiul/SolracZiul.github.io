...
function mostrarFormularioEntrega() {
  // Esconde os itens do carrinho e mostra o formulário
  document.getElementById("carrinho-itens").style.display = "none";
  document.querySelector("#carrinho-popup p").style.display = "none"; // total
  document.querySelector(".botoes-carrinho").style.display = "none";
  document.getElementById("formulario-entrega").style.display = "block";
}

function voltarParaCarrinho() {
  // Volta do formulário para os itens do carrinho
  document.getElementById("carrinho-itens").style.display = "block";
  document.querySelector("#carrinho-popup p").style.display = "block"; // total
  document.querySelector(".botoes-carrinho").style.display = "flex";
...
function enviarPedido() {
  ...
  let mensagem = "📦 *Pedido do Armazém do Sítio*%0A%0A";
  carrinho.forEach(item => {
    mensagem += `• ${item.nome} - ${item.quantidade} ${item.unidade} - R$ ${(item.preco * item.quantidade).toFixed(2)}%0A`;
  });

  const total = carrinho.reduce((s, i) => s + i.preco * i.quantidade, 0);
  mensagem += `%0ATotal: R$ ${total.toFixed(2)}%0A%0A`;

  mensagem += `👤 *Cliente:*%0ANome: ${nome}%0ACPF: ${cpf}%0APagamento: ${pagamento}%0A`;

  if (tipoEntrega === "entrega") {
    mensagem += `Telefone: ${telefone}%0AEndereço: ${endereco}%0A`;
  } else {
    mensagem += "Tipo: Retirada na loja%0A";
  }

  const link = "https://wa.me/5541999999999?text=" + encodeURIComponent(mensagem);
  window.open(link, "_blank");

  // Fecha pop-up e limpa carrinho
  document.getElementById("carrinho-popup").style.display = "none";
  document.getElementById("formulario-entrega").style.display = "none";
  document.getElementById("carrinho-itens").style.display = "block";
  document.querySelector("#carrinho-popup p").style.display = "block";
  document.querySelector(".botoes-carrinho").style.display = "flex";
